...
void geoFinalize(int ierr) {
    if (theGeometry.joukowsky_x) {free(theGeometry.joukowsky_x); theGeometry.joukowsky_x = NULL;}
    if (theGeometry.joukowsky_y) {free(theGeometry.joukowsky_y); theGeometry.joukowsky_y = NULL;}
...
void geoMeshImport() {
  ...
  //  gmshModelMeshRenumberNodes(&ierr);                        ErrorGmsh(ierr);
    ...
    gmshModelMeshGetElementsByType(3,&elem,&nElem, &node,&nNode,-1,0,1,&ierr);    ErrorGmsh(ierr);
    ...
    for (int i=0; i < n/2; i++) {
        ...
        for (size_t k = 0; k < nElementType; ++k) {
            gmshFree(elementTags[k]);
            gmshFree(nodesTags[k]);
        }
